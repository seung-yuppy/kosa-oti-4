...
.nav-pills .tab:focus {
   ...
   /* ===== 상세 공통(추가) ===== */
    ...
    .badge-role{background:#f3f4f6;color:#6b7280;border:1px solid #e5e7eb;}
    .btn-outline-gray{border-color:#e5e7eb;color:#374151;background:#fff;}
    .btn-outline-gray:hover{background:#f9fafb;border-color:#d1d5db;color:#111827;}
    .btn-brown{background:#B9761A;color:#fff;border:1px solid #B9761A;}
    .btn-brown:hover{filter:brightness(0.95);color:#fff;}
    .icon-16{width:16px;height:16px;}
    .rounded-12{border-radius:12px;}
    .post-body p{margin-bottom:1rem;}
    .count-dot::before{content:"•"; margin:0 .5rem; color:#9ca3af;}
    .post-cover{width:100%; height:auto; border:1px solid #e6e8eb; border-radius:12px;}
...
<div class="container py-5" style="max-width: 980px;">
 ...
 <!-- ===== 상세 컨텐츠 START ===== -->
  <section class="bg-white rounded-12 border p-3 p-sm-4 mb-5">
    ...
    <div class="d-flex align-items-center justify-content-between">
      <div class="d-flex align-items-center gap-3">
        <img class="avatar-40" src="<c:out value='${post.authorAvatarUrl}' default='/miniproj/resource/img/avatar-default.png'/>" alt="작성자">
        <div>
          <div class="d-flex align-items-center gap-2">
            <strong class="me-1"><c:out value="${post.authorName}"/></strong>
            <span class="badge badge-role rounded-pill px-2 py-1">일반회원</span>
          </div>
          <small class="text-muted">
            <fmt:formatDate value="${post.createdAt}" pattern="yyyy.MM.dd HH:mm"/>
            <c:if test="${post.updatedAt != null}">
              <span class="count-dot"></span>수정됨 <fmt:formatDate value="${post.updatedAt}" pattern="yyyy.MM.dd HH:mm"/>
            </c:if>
          </small>
        </div>
      </div>
      ...
      <div class="d-flex gap-2">
        <c:if test="${canEdit}">
          <a class="btn btn-outline-gray btn-sm" href="/community/${post.id}/edit">수정하기</a>
        </c:if>
        <c:if test="${canDelete}">
          <form method="post" action="/community/${post.id}/delete" onsubmit="return confirm('정말 삭제하시겠습니까?');">
    ...
    <!-- 제목 -->
    <h2 class="post-title fw-bold fs-3 mt-3 mb-3"><c:out value="${post.title}"/></h2>

    <!-- 대표 이미지 -->
    <c:if test="${not empty post.coverUrl}">
      <img class="post-cover mb-4" src="${post.coverUrl}" alt="게시글 이미지">
    </c:if>
    ...
    <div class="post-body text-muted-700" style="line-height:1.85;">
      ...
      <c:if test="${empty post.paragraphs}">
        <pre class="mb-0" style="white-space:pre-wrap;font-family:inherit;"><c:out value="${post.content}"/></pre>
      </c:if>
    </div>
    ...
    <div class="d-flex align-items-center gap-3 flex-wrap text-muted">
      ...
      <span class="d-inline-flex align-items-center gap-1">
        <svg class="icon-16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
      ...
      <span class="d-inline-flex align-items-center gap-1">
        ...
        <span><c:out value="${post.commentCount}"/></span>
      </span>

      <!-- 좋아요 -->
      <form method="post" action="/community/${post.id}/like" class="d-inline-flex align-items-center gap-1">
        <button type="submit" class="btn btn-sm btn-outline-gray d-inline-flex align-items-center gap-1">
          <svg class="icon-16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
  ...
  <section class="bg-white rounded-12 border p-3 p-sm-4">
    <h3 class="fs-5 mb-3">댓글 <span class="text-muted">(<c:out value="${post.commentCount}"/>)</span></h3>

    <!-- 댓글 작성 -->
    <c:if test="${isAuthenticated}">
      <form class="mb-4" method="post" action="/community/${post.id}/comments">
        <div class="mb-2">
      ...
      <c:forEach var="cmt" items="${post.comments}">
        <div class="list-group-item px-0">
          <div class="d-flex align-items-start gap-3">
            <img class="avatar-40" src="<c:out value='${cmt.authorAvatarUrl}' default='/miniproj/resource/img/avatar-default.png'/>" alt="">
            <div class="flex-grow-1">
              <div class="d-flex align-items-center gap-2">
                <strong><c:out value="${cmt.authorName}"/></strong>
                <small class="text-muted"><fmt:formatDate value="${cmt.createdAt}" pattern="yyyy.MM.dd HH:mm"/></small>
              </div>
              <div class="mt-1"><c:out value="${cmt.content}"/></div>
              <c:if test="${cmt.mine}">
                <div class="mt-2 d-flex gap-2">
                  <a class="btn btn-outline-gray btn-sm" href="/community/${post.id}/comments/${cmt.id}/edit">수정</a>
                  <form method="post" action="/community/${post.id}/comments/${cmt.id}/delete" onsubmit="return confirm('삭제하시겠어요?');">
  ...
  <div class="d-flex justify-content-between align-items-center mt-4">
    ...
    <div class="d-flex gap-2">
      <c:if test="${prevPost != null}">
        <a class="btn btn-outline-gray" href="/community/${prevPost.id}">이전 글</a>
      </c:if>
      <c:if test="${nextPost != null}">
        <a class="btn btn-outline-gray" href="/community/${nextPost.id}">다음 글</a>
      </c:if>
    </div>
  </div>
